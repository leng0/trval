...
export default {
    ...
    methods: {
        ...
        handleTouchMove(e) {
            if(his.isTouchLetter) {
                if(this.timer) clearTimeout(this.timer);//性能优化避免重复执行
                this.timer = setTimeout(() => {
                    let touchDistanceTop = e.touches[0].clientY; //触摸点距离顶部位置
                    let touchNum = touchDistanceTop - this.letterFirstDistanceTop;// 触摸点距离首字母的位置
                    let touchLetterIndex = Math.floor(touchNum / this.letterFirstHeight);//当前字母下标
                    if(touchLetterIndex >= 0 && touchLetterIndex < thos.letterLen) {
                       let touchLetter = this.letters[touchLetterIndex];// 
